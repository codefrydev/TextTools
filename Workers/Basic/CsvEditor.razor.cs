...
using MudBlazor;
using TextTools.Component.CSV;

namespace TextTools.Workers.Basic;
...
{ 
    ...
    private async Task OpenDialogAsync()
    {
        ...
        var data = await DialogService.ShowAsync<CsvDialogue>("Simple Dialog", options);
        var result = await data.Result;
        if (result is { Canceled: false, Data: bool })
        {
            // Handle the result here
            var file =(bool) result.Data;
            if (file )
            {
